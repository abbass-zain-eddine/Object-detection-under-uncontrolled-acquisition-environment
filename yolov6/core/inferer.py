...
class Inferer:
    def __init__(self, source, webcam, webcam_addr, weights, device, yaml, img_size, half):
        ...
        if self.device.type != 'cpu':
            self.model(torch.zeros(1, 4, *self.img_size).to(self.device).type_as(next(self.model.model.parameters())))  # warmup
    ...
    def infer(self, conf_thres, iou_thres, classes, agnostic_nms, max_det, save_dir, save_txt, save_img, hide_labels, hide_conf, view_img=True):
        ...
        for img_src, img_path, vid_cap in tqdm(self.files):
            ...
            if len(det):
                ...
                if save_txt:
                    from pathlib import Path
                    img_path_save=Path(img_path)
                    # [{"image_id": 42, "category_id": 18, "bbox": [258.15, 41.29, 348.26, 243.78], "score": 0.236}, ...
                    image_id = int(img_path_save.stem) if img_path_save.stem.isnumeric() else img_path_save.stem
                    box = self.xyxy2xywh(predn[:, :4])  # xywh
                    box[:, :2] -= box[:, 2:] / 2  # xy center to top-left corner
                    for p, b in zip(det.tolist(), box.tolist()):
                        jdict.append({'image_id': image_id,
                                    'category_id': int(p[5]),#coco91class[int(p[5])] if is_coco else int(p[5]),
    ...
    def process_image(img_src, img_size, stride, half):
        ...
        im=exposure.equalize_hist(exposure.adjust_gamma(exposure.adjust_log(im)))
        im=np.uint8(im*255)
        image = image.transpose((2, 0, 1))[::-1]  # HWC to CHW, BGR to RGB
        image = torch.from_numpy(np.ascontiguousarray(image))
        image=torch.from_numpy(np.concatenate((image,im[np.newaxis,:,:]),axis=0))
