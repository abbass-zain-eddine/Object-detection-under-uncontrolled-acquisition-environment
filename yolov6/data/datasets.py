...
class TrainValDataset(Dataset):
    ...
    #####Abbass ZEIN EDDINE#### 21-11-2022 ### I added this function that generates the edges of an image
    def fourth_binary_channel(self,im):
    ...
    def __getitem__(self, index):
        ...
        fourth_layer=self.fourth_binary_channel(img)
        # Convert
        img = img.transpose((2, 0, 1))[::-1]  # HWC to CHW, BGR to RGB
        img = np.ascontiguousarray(img)
        #img=np.uint8(exposure.equalize_adapthist(img)*255)
        fourth_layer=np.uint8(fourth_layer[0]*255)
        image_returned=torch.from_numpy(np.concatenate((img,fourth_layer[np.newaxis,:,:]),axis=0))
        return image_returned, labels_out, self.img_paths[index], shapes
